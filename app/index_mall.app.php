...
class Index_mallApp extends MallbaseApp
{
    /* 搜索商品 */
    function index()
    {
        ...
        $gcategories = $gcategory_mod->get_children($cate_id);
        $tree->setTree($gcategories, 'cate_id', 'parent_id', 'cate_name');
        $subcategory = $tree->getArrayList(0);
        $this->assign('subcategory', $subcategory);

        //分类左边导航获取
       // $this->assign('cate_fz', $this->_get_category(565));
        $this->assign('cate_tz', $this->_get_category($cate_id));
...
//        $this->assign('cate_jxsb', $this->_get_category(835));
        ...
        foreach($subcategory as $val)
        {
            ...
            $conditions = $this->_get_goods_conditions($param,$val['id']);
            $goods_mod  =& m('goods');
            ...
            $goods_list_array[] = array("cate_id"=>$val['id'],"cate_name"=>$val['value'],"goods_list"=>$goods_list);
        }

        $this->assign('goods_list', $goods_list_array);
...
//        $this->assign('display_mode', $display_mode);
    ...
    {
        $gcategory_mod =& bm('gcategory', array('_store_id' => 0));
        $ids = $gcategory_mod->get_descendant_ids($cate_id);
